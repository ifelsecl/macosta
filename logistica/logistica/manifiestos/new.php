...
<script>
$(function() {
	var $btn_guardar = $("#guardar_manifiesto").button({icons: {primary: "ui-icon-circle-check"}});
	$("#asignar").button({icons: {primary: "ui-icon-circle-check"}, text: false});
	...
	$('#form_asignar').submit(function(e) {
		...
		var id=$('#numero').val();
		$("#asignar").button('disable');
		var existe=false;
		$('.ids').each(function(index) {
			if(id == $(this).val()) existe=true;
		});
		if(existe) {
			$("#asignar").button('enable');
			return;
		}
		$.ajax({
	...
	$('#CrearPlanilla').validate({
		...
		submitHandler: function(form) {
			...
			$.ajax({
				...
				data: 'guardar=1&'+$('#CrearPlanilla').serialize()+'&'+$('.ordenable').sortable("serialize"),
				success: function(m) {
					if (m.error) {
...
<button id="regresar" class="btn btn-success pull-right" title="Regresar a Manifiestos">Regresar</button>
...
<div class="tabbable">
	...
	<div class="tab-content">
		<div class="tab-pane active" id="tab_guias">
			<div id="AsignarGuias">
				<form id="form_asignar" class="form-inline">
					<table cellpadding="0">
						<tr>
							<?php
							...
							<td><button id="asignar">Asignar</button></td>
						</tr>
					</table>
				</form>
		...
		<div class="tab-pane" id="tab_info">
			<form id="CrearPlanilla" name="CrearPlanilla" method="post" action="#">
				...
				<center class="form-actions"><button type="submit" id="guardar_manifiesto">Guardar</button></center>
